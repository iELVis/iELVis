function ntools_elecDG(varargin)

% check for spm, fsl and freesurfer
%DG if isempty(which('spm')), error('Please install SPM.\n'); end
%DG if isempty(getenv('FSLDIR')),error('Please install FSL.\n'); end
%DG if isempty(getenv('FREESURFER_HOME')), error('Please instll Freesurfer\n'); end

% get the subject info
Ssdir=getenv('SUBJECTS_DIR');
%DG Subject_path = uigetdir(Ssdir,'Select the subject FreeSurfer reconstruction folder');
%Subject_path = '/Volumes/LACIE_SHARE/SYNC/FREESURFER_TWH/TWH008';
Subject_path = '/Applications/freesurfer/subjects/TWH011';
...
if 1
    ...
    Sname='ntools_elec_V3_public'; %DG
    PathName='/Users/dgroppe/Desktop/ELEC_RECON_STUFF/NYU_LOC/ntools_elec_V3_public/'; % DG
    FileName='twh011IniCoords.txt'; %DG
end
...
%%
% DG [dep_img_file,dep_img_path] = uigetfile({'*.nii.gz';'*.nii';'*.img'},'Select the T1 pre-operation image: ',PathName);
...
%hdr = MRIread([dep_img_path dep_img_file]); %DG
...
ini_grid = elec_cell(g,:); %coordinates and labels for just grid electrodes
ini_depth = elec_cell(d,:); %coordinates and labels for just depth electrodes
elec_cell([g;d],:) = [];
...
% transform into mni space % DG ?? fix later
if 0
