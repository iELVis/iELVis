function dykstraElecPjctPersyst(sub,minimizeChange,bidsRootDir,sessionId,elecHem)
%function dykstraElecPjctPersyst(sub,minimizeChange,bidsRootDir,sessionId,elecHem)
%
...
%% read the initial text file with postimplant electrode coordinates
% Note coordinates are in voxels (NOT RAS)
postimpLocFname=fullfile(elecReconDir,[sub 'PostimpLoc.txt']);
...
%% Create coord matrix, finddepth electrodes, identify hemisphere
nElec=size(elec_cell,1);
elecMatrix=zeros(nElec,3); %LIP voxel coordinates; indexing starts at 1 for MATLAB
isDepth=ones(nElec,1);
elecHem=zeros(nElec,1); %Which hemisphere each electrode is in/over, Left=1, Right=0;
elecType=cell(nElec,1); % D S or G for depth, strip, or grid
elecStems=cell(nElec,1); % Full contact name (e.g., LHD1)
elecNums=zeros(nElec,1); % Just the contact # 
for a=1:nElec
    elecType{a}=elec_cell{a,7}; 
    if lower(elec_cell{a,7})~='d'
        isDepth(a)=0;
    end
    for b=1:3
        elecMatrix(a,b)=elec_cell{a,b+2}+1; % $$@@ we add 1 so that indexing starts at 1
    end
    if lower(elec_cell{a,6})=='l'
        elecHem(a)=1;
    end
    elecNums(a)=elec_cell{a,2}(1);
    elecStems{a}=[elec_cell{a,1} num2str(elecNums(a))];
end
