...
#The second argument is the full path to the Patient's folder of MNI files (e.g., /Users/Penfield/Desktop/PT001_MNI). It should contain pre- and post-implant scans in MNI space as well as ElecInfo.tsv, a tab separated text file of electrode coordinates, and ElecPairs.tsv, a tab separated text file that indicates which electrodes are neighbors and their color.
#
#This script uses FSL's flirt command to rigidly (i.e., 6 degrees of freedom mapping) transform the MNI aligned preimplant MRI so that it aligns with the FreeSurfer MRI. It then applies that same transformation to the postimplant scan and electrode coordinates. The result is an elec_recon subfolder in the patient's FreeSurfer folder that is then ready for iELVis's MATLAB plotting functions.
...
usage='\nUSAGE:\n  mni2fsurf.sh FsurfId mniFolder\n\nEXAMPLE:\n mni2fsurf.sh PT001 /Users/Penfield/Desktop/PT001_MNI\n'
