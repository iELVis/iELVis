...
function [csv_matrix, csv_ct]=csv2CellOneCol(csv_fname,delimiter,header_lines,col)
...
while 1
    ...
    while ~isempty(tline),
        ...
        if col_ct==col
           id=findstrInCell(t,csv_matrix,0);
           if isempty(id)
               csvLen=csvLen+1;
               csv_matrix{csvLen,1}=t;
               csv_ct(csvLen)=uint64(1);
           else
               csv_ct(id)=csv_ct(id)+uint64(1);
           end
        end
        col_ct=col_ct+1;
    end
